...
func Aria2(w http.ResponseWriter, r *http.Request) {
	...
	case "aria2.addUri":
		CreateTask(w, r)
	default:
		WriteJson(w, Json{"id": gjson.GetBytes(body, "id").String(), "jsonrpc": "2.0", "result": ""})
	}
	//r.Body = io.NopCloser(bytes.NewReader(task))
}
...
func CreateTask(w http.ResponseWriter, r *http.Request) {
	...
	if method == "aria2.addUri" {
		params := gjson.GetBytes(body, "params").Array()
		var (
			url       string
			name      string
			path      string
		...
		for _, param := range params {
			if param.Type.String() == "JSON" {
				if strings.Contains(param.String(), "{") && strings.Contains(param.String(), "}") {
					path = gjson.Get(param.String(), "dir").String()
					name = gjson.Get(param.String(), "out").String()
					headers := gjson.Get(param.String(), "header").Array()
					for _, header := range headers {
						split := strings.SplitN(header.String(), ":", 2)
						switch strings.ToLower(split[0]) {
						case "referer":
							referer = strings.Trim(split[1], " ")
						case "user-agent":
							userAgent = strings.Trim(split[1], " ")
						}
					}
				} else {
					url = strings.Trim(strings.Trim(param.String(), `["`), `"]`)
				}
			}
		}
		task, _ := json.Marshal(Json{
			"req": Json{
				"url": url,
				"extra": Json{
					"header": Json{
	...
	if ReadJson(r, w, &req) {
		...
		if method == "aria2.addUri" {
			WriteJson(w, Json{"id": gjson.GetBytes(body, "id").String(), "jsonrpc": "2.0", "result": taskId})
		} else {
