...
func (d *HuYa) link(ctx context.Context, file model.Obj) (*model.Link, error) {
	...
	url, _, err := d.getStreamUrl("https://mp.huya.com/cache.php?do=profileRoom&m=Live&roomid="+roomId, false)
	if err != nil {
...
func (d *HuYa) Other(ctx context.Context, args model.OtherArgs) (interface{}, error) {
	roomId := d.roomIdToString(args.Obj.GetID())
	_, lists, err := d.getStreamUrl("https://mp.huya.com/cache.php?do=profileRoom&m=Live&roomid="+roomId, true)
	if err != nil {
		log.Errorf("%s:%s", args.Obj.GetName(), err)
		return nil, fmt.Errorf("%s:%s", args.Obj.GetName(), err)
	}
	return VideoPreview{VideoPreviewPlayInfo: VideoPreviewPlayInfo{LiveTranscodingTaskList: lists}}, nil
}
