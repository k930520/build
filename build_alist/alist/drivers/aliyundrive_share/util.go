		...
		for i, file := range resp.Items {
			if file.Type == "file" {
				if file.Category == "doc" {
					resp.Items[i].Category = "video"
					resp.Items[i].FileExtension = "mp4"
					resp.Items[i].Name = strings.Replace(file.Name, "."+file.FileExtension, "_", 1) + file.UpdatedAt.Local().Format("2006-01-02") + ".mp4"
				}
				if file.Category == "video" {
					resp.Items[i].Name = strings.Replace(file.Name, "."+file.FileExtension, "_", 1) + file.UpdatedAt.Local().Format("2006-01-02") + "." + file.FileExtension
				}
			} else {
			...
			if strings.Contains(shareId, "?pwd=") {
				resp.Items[i].ShareId = file.ShareId + "?pwd=" + pwd
			}
		}
...
func (d *AliyundriveShare) getFiles(ctx context.Context, args map[string]string) ([]File, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "share_link") {
			return nil, fmt.Errorf("%s,%s", err.Error(), "https://www.alipan.com/s/"+args["id"])
		}
...
func (d *AliyundriveShare) copyFile(shareId string, fileId string) (string, error) {
	...
	data := `{"requests":[{"body":{"file_id":"` + fileId + `","share_id":"` + id + `","auto_rename":true,"to_parent_file_id":"` + d.RootID.GetRootId() + `","to_drive_id":"` + d.DriveId + `"},"headers":{"Content-Type":"application/json"},"id":"0","method":"POST","url":"/file/copy"}],"resource":"file"}`
	var result BatchResult
	_, err := d.request(shareId, "https://api.aliyundrive.com/adrive/v4/batch", http.MethodPost, func(req *resty.Request) {
...
func (d *AliyundriveShare) getFileUrl(fileId string) (string, error) {
	...
	data := `{"requests":[{"body":{"drive_id":"` + d.DriveId + `","file_id":"` + fileId + `"},"headers":{"Content-Type":"application/json"},"id":"0","method":"POST","url":"/file/delete"}],"resource":"file"}`
	_, _ = d.request("", "https://api.aliyundrive.com/adrive/v4/batch", http.MethodPost, func(req *resty.Request) {
		req.SetBody(data)
	})
	if err != nil {
		return "", err
